...
function computeTreeData(text){
    ...
    var codes = computeCodes(tree);
    var codesSorted = sortCodes(codes);
    var treeData = buildTreeData(treeData, tree, codesSorted);
    return treeData;
  }
  ...
  function buildTree(sortedFreq){
    ...
    for(var i = 0; i < sortedFreq.length; i++){
      tree.push([sortedFreq[i][0], sortedFreq[i][1]]);
    }
    while(tree.length > 1){
  ...
  function sortCodes(codes){
    ...
    for(var key in codes){
      codesSorted.push([key, codes[key]]);
    }
  ...
  function buildTreeData(treeData, tree, codesSorted){
    var stack = [];
    stack.push([tree, treeData]);
    while(stack.length > 0){
      var node = stack.pop();
      if(node[0].length == 2){
    ...
    for(var i = 0; i < codesSorted.length; i++){
      treeData.children.push({"name": codesSorted[i][0] + " = " + codesSorted[i][1]});
    }
    return treeData;
  }
